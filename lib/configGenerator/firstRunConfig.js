...
async function firstRunConfig(
  ...
  while (mixerCountYml > mixerCountJson) {
    ...
    if (mixerCountYml >= lastMixerNumber) {
      console.error('Error: Deletion did not reduce count, breaking loop');
      break;
    }
  }
  ...
  console.log(`Updated ${networkName}.yml - mixers: ${Object.keys(ansibleYml.mixers).length}, load_testers: ${Object.keys(ansibleYml.load_testers).length}`);
}
