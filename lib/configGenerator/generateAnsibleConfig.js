...
async function generateAnsibleConfig(
  ...
  const {
    hdPrivateKey: dpns_HDPrivateKey,
    derivedMasterPrivateKey: dpns_DerivedMasterPK,
    derivedSecondPrivateKey: dpns_DerivedSecondPK,
  } = await generateHDPrivateKeys(network);
  const {
    hdPrivateKey: dashpay_HDPrivateKey,
    derivedMasterPrivateKey: dashpay_DerivedMasterPK,
    derivedSecondPrivateKey: dashpay_DerivedSecondPK,
  } = await generateHDPrivateKeys(network);
  const {
    hdPrivateKey: feature_flags_HDPrivateKey,
    derivedMasterPrivateKey: feature_flags_DerivedMasterPK,
    derivedSecondPrivateKey: feature_flags_DerivedSecondPK,
  } = await generateHDPrivateKeys(network);
  const {
    hdPrivateKey: mn_reward_shares_HDPrivateKey,
    derivedMasterPrivateKey: mn_reward_shares_DerivedMasterPK,
    derivedSecondPrivateKey: mn_reward_shares_DerivedSecondPK,
  } = await generateHDPrivateKeys(network);
  const {
    hdPrivateKey: withdrawals_HDPrivateKey,
    derivedMasterPrivateKey: withdrawals_DerivedMasterPK,
    derivedSecondPrivateKey: withdrawals_DerivedSecondPK,
  } = await generateHDPrivateKeys(network);
  ...
  // HD private keys
  config.dpns_hd_private_key = dpns_HDPrivateKey.toString();
  config.dpns_hd_master_public_key = dpns_DerivedMasterPK.privateKey.toPublicKey().toString();
  config.dpns_hd_second_public_key = dpns_DerivedSecondPK.privateKey.toPublicKey().toString();

  config.dashpay_hd_private_key = dashpay_HDPrivateKey.toString();
  config.dashpay_hd_master_public_key = dashpay_DerivedMasterPK.privateKey.toPublicKey().toString();
  config.dashpay_hd_second_public_key = dashpay_DerivedSecondPK.privateKey.toPublicKey().toString();

  config.feature_flags_hd_private_key = feature_flags_HDPrivateKey.toString();
  config.feature_flags_hd_master_public_key = feature_flags_DerivedMasterPK.privateKey.toPublicKey().toString();
  config.feature_flags_hd_second_public_key = feature_flags_DerivedSecondPK.privateKey.toPublicKey().toString();

  config.mn_reward_shares_hd_private_key = mn_reward_shares_HDPrivateKey.toString();
  config.mn_reward_shares_hd_master_public_key = mn_reward_shares_DerivedMasterPK.privateKey.toPublicKey().toString();
  config.mn_reward_shares_hd_second_public_key = mn_reward_shares_DerivedSecondPK.privateKey.toPublicKey().toString();

  config.withdrawals_hd_private_key = withdrawals_HDPrivateKey.toString();
  config.withdrawals_hd_master_public_key = withdrawals_DerivedMasterPK.privateKey.toPublicKey().toString();
  config.withdrawals_hd_second_public_key = withdrawals_DerivedSecondPK.privateKey.toPublicKey().toString();
  ...
  config.platform_initial_core_chain_locked_height = 4040;
