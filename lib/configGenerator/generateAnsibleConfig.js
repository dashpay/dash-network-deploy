...
async function generateAnsibleConfig(network, networkName, masternodesCount, seedsCount) {
  ...
  const { hdPrivateKey: dpns_HDPrivateKey, derivedPrivateKey: dpns_DerivedPK } = await generateHDPrivateKeys(network);
  const { hdPrivateKey: dashpay_HDPrivateKey, derivedPrivateKey: dashpay_DerivedPK } = await generateHDPrivateKeys(network);
  const { hdPrivateKey: feature_flags_HDPrivateKey, derivedPrivateKey: feature_flags_DerivedPK } = await generateHDPrivateKeys(network);
  const { hdPrivateKey: mn_reward_shares_HDPrivateKey, derivedPrivateKey: mn_reward_shares_DerivedPK } = await generateHDPrivateKeys(network);
  ...
  config.dpns_hd_public_key = dpns_DerivedPK.privateKey.toPublicKey().toString();
  config.dashpay_hd_private_key = dashpay_HDPrivateKey.toString()
  config.dashpay_hd_public_key = dashpay_DerivedPK.privateKey.toPublicKey().toString();
  config.feature_flags_hd_private_key = feature_flags_HDPrivateKey.toString()
  config.feature_flags_hd_public_key = feature_flags_DerivedPK.privateKey.toPublicKey().toString();
  config.mn_reward_shares_hd_private_key = mn_reward_shares_HDPrivateKey.toString();
  config.mn_reward_shares_hd_public_key = mn_reward_shares_DerivedPK.privateKey.toPublicKey().toString();
