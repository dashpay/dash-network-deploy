...
const { contractId: dpnsContractId, ownerId: dpnsOnwerId } = require('@dashevo/dpns-contract/lib/systemIds');
const { contractId: dashpayContractId, ownerId: dashpayOnwerId } = require('@dashevo/dashpay-contract/lib/systemIds');
const { contractId: featureFlagsContractId, ownerId: featureFlagsOnwerId } = require('@dashevo/feature-flags-contract/lib/systemIds');
const { contractId: masternodeRewardSharesContractId, ownerId: masternodeRewardSharesOnwerId } = require('@dashevo/masternode-reward-shares-contract/lib/systemIds');
...
async function generateAnsibleConfig(network, networkName, masternodesCount, seedsCount) {
  ...
  config.platform_dpns_contract_id = dpnsContractId;
  config.platform_dpns_contract_block_height = '';
  config.platform_dpns_top_level_domain_identity = dpnsOnwerId;
  config.platform_dashpay_contract_id = dashpayContractId;
  config.platform_dashpay_contract_block_height = '';
  config.platform_dashpay_top_level_domain_identity = dashpayOnwerId;
  config.platform_feature_flags_contract_id = featureFlagsContractId;
  config.platform_feature_flags_contract_block_height = '';
  config.platform_feature_flags_top_level_identity = featureFlagsOnwerId;
  config.dpns_hdprivkey = '';
  config.dpns_hdpubkey = '';
  config.dashpay_hdprivkey = '';
  config.dashpay_hdpubkey = '';
  config.ff_hdpubkey = '';
  config.ff_hdprivkey = '';
  config.mnrewardshares_hdpubkey = '';
  config.mnrewardshares_hdprivkey = '';

  const { hdPrivateKey: dpnsHDPrivateKey, derivedPrivateKey: dpnsDerivedPK } = await generateHDPrivateKeys(network);
  const { hdPrivateKey: dashpayHDPrivateKey, derivedPrivateKey: dashpayDerivedPK } = await generateHDPrivateKeys(network);
  const { hdPrivateKey: ffHDPrivateKey, derivedPrivateKey: ffDerivedPK } = await generateHDPrivateKeys(network);
  const { hdPrivateKey: mnrewardsharesHDPrivateKey, derivedPrivateKey: mnrewardsharesDerivedPK } = await generateHDPrivateKeys(network);
  ...
  // HD private keys
  config.dpns_hdprivkey = dpnsHDPrivateKey.toString();
  config.dpns_hdpubkey = dpnsDerivedPK.privateKey.toPublicKey().toString();
  config.dashpay_hdprivkey = dashpayHDPrivateKey.toString()
  config.dashpay_hdpubkey = dashpayDerivedPK.privateKey.toPublicKey().toString();
  config.ff_hdprivkey = ffHDPrivateKey.toString()
  config.ff_hdpubkey = ffDerivedPK.privateKey.toPublicKey().toString();
  config.mnrewardshares_hdprivkey = mnrewardsharesHDPrivateKey.toString();
  config.mnrewardshares_hdpubkey = mnrewardsharesDerivedPK.privateKey.toPublicKey().toString();
