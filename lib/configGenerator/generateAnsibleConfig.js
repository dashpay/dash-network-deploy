...
async function generateAnsibleConfig(network, networkName, masternodesCount, seedsCount) {
  ...
  for (let i = 1; i <= masternodesCount; i++) {
    ...
    const Bls = await BlsSignatures();

    const randomBytes = new Uint8Array(crypto.randomBytes(256));
    const operatorPrivateKey = Bls.AugSchemeMPL.key_gen(randomBytes);
    ...
    config.masternodes[`masternode-${i}`] = {
      ...
      operator: {
        public_key: Bls.Util.hex_str(operatorPublicKey.serialize()),
        private_key: Bls.Util.hex_str(operatorPrivateKey.serialize()),
      },
    };
