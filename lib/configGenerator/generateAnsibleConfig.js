...
async function generateAnsibleConfig(
  ...
  config.dashmate_helper_image = 'dashpay/dashmate-helper';
  ...
  //Piggybacking off this to generate mixer keys, these are not masternodes. Should we create a new function?
  //Does it matter?
  config.mixers = {};

  for (let i = 1; i <= mixerCount; i++) {
    config.mixers[`mixer-${i}`] = await generateDip3Keys(network);
  }
