...
async function generateAnsibleConfig(network, networkName, masternodesCount, seedsCount) {
  ...
  function generateTenderdashNodeKeys() {
    ...
    // Strip static metadata from keys so they are 32 byts and
    ...
    const cachedKey = Buffer.from([...privateKey.slice(16), ...publicKey.slice(12)]);

    // Derive ID from key
    const publicKeyHash = crypto.createHash('sha256')
      .update(Buffer.from([...publicKey.slice(12)]))
      .digest('hex');
  ...
  config.tenderdash_nodes = {};

  for (let i = 1; i <= masternodesCount; i++) {
    config.tenderdash_nodes[`masternode-${i}`] = {
      node: generateTenderdashNodeKeys(),
    };
  }

  for (let i = 1; i <= seedsCount; i++) {
    config.tenderdash_nodes[`seed-${i}`] = {
      node: generateTenderdashNodeKeys(),
    };
  }
