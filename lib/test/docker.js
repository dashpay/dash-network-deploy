...
const createDocker = (hostName, options = {}) => (new Docker({
  host: hostName,
  port: 2375,
  timeout: 60 * 1000,
  ...options,
...
const execJSONDockerCommand = async (docker, containerId, cmd) => {
...
const execDockerCommand = async (docker, containerId, cmd) => {
  const container = docker.getContainer(containerId);
